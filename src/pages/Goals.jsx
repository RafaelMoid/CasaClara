  ...
  return (
    <>
        ...
        {goals.map((goal) => {
          ...
          return (
            <article className="card flex gap-3" key={goal.id}>
              <div className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-brand-50 text-brand-700 dark:bg-brand-950 dark:text-brand-100">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="progress-track">
                  <div className="h-full rounded-full bg-brand-600 transition-[width] duration-slow" style={{ width: `${percent}%` }} />
                </div>
              </div>
            </article>
          );
      ...
      <form className="card mt-4" onSubmit={submit}>
        ...
        <input className="input mb-3" inputMode="decimal" type="number" min="0" step="0.01" placeholder="Valor alvo" value={form.target} onChange={(event) => setForm({ ...form, target: event.target.value })} required />
        <input className="input mb-3" inputMode="decimal" type="number" min="0" step="0.01" placeholder="Valor atual" value={form.current} onChange={(event) => setForm({ ...form, current: event.target.value })} required />
